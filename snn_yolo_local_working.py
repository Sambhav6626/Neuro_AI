    ...
    def _initialize_weights(self):
            for m in self.modules():
                if isinstance(m, nn.Conv2d):
                    ...
                    nn.init.constant_(m.weight, 0.1)  # Changed from 1.0
                    nn.init.zeros_(m.bias)
...
def train_snn_model(model, train_loader, val_loader, epochs=20, lr=0.001, 
    ...
    """Improved SNN training with proper YOLO loss"""
    
    print("\n" + "="*80)
    print("STEP 5: TRAINING SNN MODEL (IMPROVED WITH YOLO LOSS)")
    print("="*80)
...
# ============================================================================
def decode_snn_outputs(outputs, conf_threshold=0.25, imgsz=640, num_classes=20):
    """
    Decode SNN spike outputs into bounding boxes
...
def evaluate_snn(model, val_loader, timesteps=4, device=None, max_batches=None):
    ...
    with torch.no_grad():
        for batch_idx, (images, targets) in enumerate(tqdm(val_loader, desc="SNN Eval")):
            ...
            # 🔧 FIXED: Properly decode predictions and add to lists
...
def run_complete_pipeline(voc_path, batch_size=16, epochs=50, eval_batches=None, 
    ...
    if train_from_scratch:
        ...
        # 👇 NEW: Generate YOLOv8 training plots
   ...
   # =========================================================================
    ...
    snn_model, train_losses, firing_rates, snn_train_energy = train_snn_model(  # 👈 CHANGED
        snn_model,
        train_loader,
        val_loader,
        epochs=epochs,
    ...
    # 👇 NEW: Generate SNN training plots
